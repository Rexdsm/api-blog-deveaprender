...
def token_obrigatorio(f):
    ...
    def decorated(*args, **kwargs):
        token = None
        #Verificar se o um token foi enviado
        if 'x-access-token' in request.headers:
            request.headers['x-access_token']
        if not token:
        ...
        try:
            ...
            return jsonify({'mensage': 'Token é invalido'},401)
        return f(autor, *args, **kwargs)
    return decorated


#Criando login e colocando a API com segurança 
@app.route('/login')
def login():
...
# NOVA API para autores
# http://localhost:5000/autores
    
@app.route('/autores')
@token_obrigatorio
def obter_autores(autor):
    autores = Autor.query.all() #Extraindo todos os dados que tem dentro daquela tabela 
    lista_de_autores = []
...
def obter_autor_por_id(autor, id_autor):
    ...
    # return jsonify(f'Você buscou pelo autor {autor_atual}') # forma normal mas a forma mais identada e correte é essa abaixo
...
def excluir_autor(autor, id_autor):
    ...
    if not autor_existente:
        return jsonify({'mensagem': 'Esse autor não existe no banco de dados'})
